...
def activeSpreadsheetTreeView():
  ...
  if len(spreadsheetViews) > 0:
    return spreadsheetViews[0]
  else:
    return None
...
class SpreadsheetExportCSVAction(QAction):
  ...
  def exportCSVFromActiveSpreadsheetView(self):
    ...
    csvSavePath, filter = QFileDialog.getSaveFileName(None, caption="Choose CSV File to Save to", directory=os.path.expanduser("~"), filter="CSV Files (*.csv)")
#     savePath = csvSavePath
    savePath = os.path.join(os.path.expanduser("~"), "Desktop", "SpreadsheetExport%d.csv"%randint(0,9999))
